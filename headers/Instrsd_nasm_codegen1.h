...
static inline void name##_XREG_XREG_NASM(Dynamic_array* array, int reg_dst, int reg_src) { \
...
\
static inline void name##_XREG_MEM_NASM(Dynamic_array* array, int reg, int mem_reg, int32_t disp) { \
    INSERT_STRING_CONSTANT(array, "\t" #name " "); \
...
static inline void name##_MEM_XREG_NASM(Dynamic_array* array, int mem_reg, int32_t disp, int xmm_reg) { \
    INSERT_STRING_CONSTANT(array, "\t" #name " ["); \
    INSERT_STRING(array, Registers_names[mem_reg]); \
    INSERT_STRING_CONSTANT(array, " + "); \
    ...
    int symbols_written = sprintf(array->data + array->size, "%#x], ", disp);\
    array->size += symbols_written;\
    INSERT_STRING(array, X_Registers_names[xmm_reg]); \
    INSERT_STRING_CONSTANT(array, "\n"); \
}
